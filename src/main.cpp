...
#if defined(NDEBUG)
# error "Circcash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7c9f9446c1cae87d7bf57755e01b2684ad06828d22069464829a97c58982aedc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 3); // Circcash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Circcash Signed Message:\n";
...
{
    // Circcash: IsDust() detection disabled, allows any valid dust to be relayed.
...
//    nSubsidy >>= (nHeight / 840000); // Circcash: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.0 * 24 * 60 * 60; // Circcash: 3.5 days
static const int64 nTargetSpacing = 2.0 * 60; // Circcash: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Circcash: This fixes an issue where a 51% attack can change difficulty at will.
...
std::string developerWallet="CZM9CzT6J4ZEivuRfB817X7yzcGuRN3UqM";
CTxDestination developerWalletDest=CBitcoinAddress(developerWallet).Get();
CScript developerCScript;
developerCScript.SetDestination(developerWalletDest);


//txNew.vout[1].scriptPubKey=scriptPubKeyIn;
//jvanname CScript developerCScript=GetScriptForDestination(developerWalletDest);

if (vtx[0].vout.size()<2){return state.DoS(100, error("ConnectBlock() : bad-txns-vout-sizeinvalid"));}


if (vtx[0].vout[1].scriptPubKey!=developerCScript){
return state.DoS(100, error("ConnectBlock() : bad-txns-vout-fundoutputinvalid"));
}

if ((vtx[0].vout[1].nValue * 7<vtx[0].vout[0].nValue)&&(((pindex->nHeight)<1048576)||((pindex->nChainWork)<(uint256(1)<<74)))){
return state.DoS(100, error("ConnectBlock() : bad-txns-vout-fundoutputtoosmall"));}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Circcash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x708bb3990020c7d6862ad7f7d302fa2ded9d98dbe64629965402449583acc2df");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Saharan dust storm a couple days ago 7/2/20";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a5b059f026c994a756de6f7434571460ed820cefa965f51d912d410b011f61ec41de2bb3132a9489545231d1a7652989bb22e2681d2cf46c57d7b5c316a3e17a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1593618626;
        block.nBits    = 0x1f2ffff0;
        block.nNonce   = 2132001519;

        if (fTestNet)
        {
            block.nTime    = 1593618627;
            block.nNonce   = 435971643;
        }
...
//#endif
		 hashspinfinal(BEGIN(block.nVersion), block.nNonce,BEGIN(thash));
                if (thash <= hashTarget)
                    break;
        ...
        assert(block.hashMerkleRoot == uint256("0x8ec4d5e54c925aebf6393cabdc3ad1ba583a65b340003dcdf71a32d009f4350e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x62, 0x82, 0x0b, 0xc6 }; // Circcash: increase each by adding 2 to bitcoin's value.
...
txNew.vout[1].scriptPubKey=developerCScript;
//jvanname CScript developerCScript=GetScriptForDestination(developerWalletDest);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CirccashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CirccashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CirccashMiner(CWallet *pwallet)
{
    printf("CirccashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("circcash-miner");
    ...
    try { loop {
        ...
        printf("Running CirccashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
////////////////
    ...
    {
        printf("CirccashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CirccashMiner, pwallet));
}
